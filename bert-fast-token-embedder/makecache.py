...
max_piecelength = max(map(lambda x: len(tokenizer.tokenize(x)), cachewords))
...
for batch in batches:
    ...
    for i, seq in enumerate(batch):
        for token, begin, end in token_ranges(seq):
            ...
            tensor = torch.cat(
                [output[i].narrow(0, 1 + begin, end - begin + 1),
                 torch.zeros(max_piecelength - end + begin - 1, 13, 768)], 0)
